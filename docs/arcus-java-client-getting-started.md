...
ARCUS는 오픈소스 memcached를 기반으로 데이터 분산과 fault-tolerance를 구현한 메모리 기반의 캐시 클러스터 입니다.
* memcached : 구글, 페이스북 등에서 대규모로 사용하고 있는 메모리 캐시 서버입니다.
* 캐시 : 자주 사용되는 데이터를 고속의 저장소에 넣어둠으로써, 느린 저장소로의 요청을 줄이고 보다 빠른 응답성을 제공할 수 있는 서비스입니다.
* 메모리 기반 : ARCUS는 데이터를 메모리에만 저장합니다. 따라서 모든 데이터는 휘발성이며 언제든지 삭제될 수 있습니다.
* 클러스터 : 각 서비스는 초기 캐시 클러스터를 구성한 후 동적으로 캐시 서버를 추가하거나 삭제할 수 있습니다. (단, 일부 데이터는 유실됩니다)
* fault-tolerance : ARCUS는 캐시 서버들의 이상 상태를 검사하면서 고장 서버를 탐지, 제거하여 정상 캐시 서버만으로 캐시 서비스를 지속합니다.

또한 ARCUS는 key-value 형태의 데이터뿐만 아니라 하나의 키로 List, Set, Map, B+Tree 등의 자료구조를 저장할 수 있는 Collection 기능을 제공합니다.

## 미리 알아두기

- 키(key)
    - ARCUS의 key는 prefix와 subkey로 구성되며, 이를 콜론(:)으로 구분합니다. (예) *users:user_12345*
    - ARCUS는 prefix 기준의 통계를 수집합니다. prefix 개수의 제한은 없으나 통계 수집이 필요할 경우는 많지 않은 개수(20개 이하)로 생성하는 것을 권합니다.
...
* ARCUS 접속 정보
    - ARCUS admin: ZooKeeper 주소이며, 캐시 서버들의 `IP:PORT` 정보를 조회하고 변경이 있을 때 클라이언트에게 알려주는 역할을 합니다.
    - ARCUS service code: 사용자 또는 응용에게 제공된 ARCSU 캐시 클러스터를 각각 구분하는 식별자입니다.
