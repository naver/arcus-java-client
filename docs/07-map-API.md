...
Map에서 element를 삭제하는 함수들은 세 유형이 있다.
...
셋째, Map에서 주어진 mkeyList의 element를 삭제한다.

```java
CollectionFuture<Boolean>
asyncMopDelete(String key, List<String> mkeyList, boolean dropIfEmpty)
```

- key: 삭제 대상 map의 key
- mkeyList: 삭제할 element의 mkeyLists
...
다음은 map에서 mkey가 mkey1, mkey2인 element를 삭제하는 예제이다.
...
try {
    future = client.asyncMopDelete(key, mkeyList, dropIfEmpty); // (1)
} catch (IllegalStateException e) {
    // handle exception
}
...
1. Map에서 mkeyList에 해당하는 element 들을 삭제한다.
